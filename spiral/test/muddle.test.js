...
const FIXTURE_YAML = path.resolve(__dirname, 'fixtures/test-100.yaml');

function run(...args) {
  return spawnSync(process.execPath, [CLI, ...args], { encoding: 'utf8' });
}
function tmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'vp-sp-muddle-'));
}
...
test('muddle: output has hashed:true, boardHash, no answer: or hash: in inward or outward', () => {
  const dir   = tmpDir();
  const input = path.join(dir, 'test-100.yaml');
  fs.copyFileSync(FIXTURE_YAML, input);
  run('muddle', input);
  const muddled = yaml.load(fs.readFileSync(path.join(dir, 'test-100.muddled.yaml'), 'utf8'));

  assert.equal(muddled.hashed,    true);
  assert.equal(muddled.boardHash, EXPECTED_BOARD_HASH);
  for (const entry of [...muddled.inward, ...muddled.outward]) {
...
test('muddle: building from muddled YAML produces same boardHash as source built with hashed:true', () => {
  const dir         = tmpDir();
  const input       = path.join(dir, 'test-100.yaml');
  const muddledPath = path.join(dir, 'test-100.muddled.yaml');
  const outHtml     = path.join(dir, 'test-100.muddled.html');
  fs.copyFileSync(FIXTURE_YAML, input);
  run('muddle', input);
...
test('validate: muddled spiral YAML exits 0 with skip notice', () => {
  const dir   = tmpDir();
  const input = path.join(dir, 'test-100.yaml');
  fs.copyFileSync(FIXTURE_YAML, input);
  run('muddle', input);
  const r = run('validate', path.join(dir, 'test-100.muddled.yaml'));
